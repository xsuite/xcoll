...
def test_loading():
    ...
    colldb_2 = xc.CollimatorDatabase.from_yaml(path / 'colldb_lhc_run3_b1.yaml')
    colldb_3 = xc.CollimatorDatabase.from_yaml(path / 'colldb_lhc_run3_b1_no_merge.yaml')
    colldb_4 = xc.CollimatorDatabase.from_json(path / 'colldb_lhc_run3_b1.json')
    with open(path / 'colldb_lhc_run3_b1.json', 'r') as fid:
        colldb_5 = xc.CollimatorDatabase.from_dict(json.load(fid))
    colldb_6 = xc.CollimatorDatabase.from_yaml(path / 'colldb_lhc_run3_b1_no_families.yaml', nemitt_x=3.5e-6, nemitt_y=3.5e-6)
    colldb_7 = xc.CollimatorDatabase.from_SixTrack(path / 'colldb_lhc_run3_b1.dat', nemitt_x=3.5e-6, nemitt_y=3.5e-6)
