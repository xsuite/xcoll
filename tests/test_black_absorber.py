...
def test_horizontal_parallel():
    jaw_L, jaw_R, _, _, cox, _, L, coll = _make_absorber()
...
def test_horizontal_with_tilts():
    jaw_L, jaw_R, jaw_B_L, jaw_B_R, cox, _, L, coll = _make_absorber(tilsthift=[0.0005, -0.00015])
    part, x, _, xp, _ = _generate_particles(four_dim=True)
    coll.track(part)
    part.reshuffle()
    # Particles in front of the jaw are lost, ...
...
def test_vertical_parallel():
    jaw_L, jaw_R, _, _, _, coy, L, coll = _make_absorber(angle=90)
...
def test_vertical_with_tilts():
    jaw_L, jaw_R, jaw_B_L, jaw_B_R, _, coy, L, coll = _make_absorber(angle=90, tilsthift=[0.0005, -0.00015])
    part, _, y, _, yp = _generate_particles(four_dim=True)
    coll.track(part)
    part.reshuffle()
    # Particles in front of the jaw are lost, ...
...
def test_angled_parallel():
    angle=17.8
    jaw_L, jaw_R, _, _, cox, _, L, coll = _make_absorber(angle=angle, rotate_co=True)
...
def test_angled_with_tilts():
    angle=17.8
    jaw_L, jaw_R, jaw_B_L, jaw_B_R, cox, _, L, coll = _make_absorber(angle=angle, tilsthift=[0.0005, -0.00015], rotate_co=True)
    part, x, _, xp, _ = _generate_particles(four_dim=True,angle=angle)
    coll.track(part)
    part.reshuffle()
    # Particles in front of the jaw are lost, ...
...
def _make_absorber(angle=0, tilsthift=[0,0], rotate_co=False):
    jaws = [0.03, -0.01]
    jaws_B = jaws + tilsthift
    co = [0.0075, -0.089]
    L = 0.873
    coll = xc.BlackAbsorber(active_length=L, angle=angle, dx=co[0], dy=co[1],
