...
def test_gaps(beam, test_context):
    ...
    oldval = np.array([None, None, None, None, 127.5, 0.0])
    ...
    for r, i in enumerate(second_gapL):
        check_3_times(coll, newval_gapL, oldval, np.array(['gap_L', i]), r)
    ...
    for r, i in enumerate(second_gapR):
        check_3_times(coll, newval_gapR, oldval, np.array(['gap_R', i]), r)
    ...
    for r, i in enumerate(second_jawL):
        check_3_times(coll, newval_jawL, oldval, np.array(['jaw_L', i]), r)
    ...
    for r, i in enumerate(second_jawR):
        check_3_times(coll, newval_jawR, oldval, np.array(['jaw_R', i]), r)
    ...
    for r, i in enumerate(second_tilt):
        check_3_times(coll, newval_tilt, oldval, np.array(['tilt', i]), r)
    ...
    for r, i in enumerate(second_angle):
        check_3_times(coll, newval_angle, oldval, np.array(['angle', i]), r)
...
def check_3_times(coll, newval, oldval, succession, round):
    """
    ...
    if not succession[0].startswith('t'):
        assert coll.tilt == oldval[5]
    if not succession[0].startswith('a'):
        assert coll.angle == oldval[4]
    if succession[0] in third:
        third = np.delete(third, np.where(third == succession[0]))
    oldval[:] = [coll.jaw_L, coll.jaw_R, coll.gap_L, coll.gap_R, coll.angle, coll.tilt]
    ...
    if (succession[0].startswith('j') and succession[1].startswith('g')):
        if (succession[0].endswith('L') and succession[1].endswith('R')):
            no_gapL = True
            assert coll.jaw_L > coll.jaw_R
            assert coll.gap_L == 999.
            assert np.isclose(coll.jaw_R, (coll.gap_R * coll.sigma[0][1]) + coll.co[0][1], atol=1e-6)
            assert np.isclose(coll.jaw_L, (coll.gap_L * coll.sigma[0][0]) + coll.co[0][0], atol=1e-6)
        elif (succession[0].endswith('R') and succession[1].endswith('L')):
            no_gapR = True
            assert coll.jaw_L > coll.jaw_R
            assert coll.gap_R == -999.
            assert np.isclose(coll.jaw_R, (coll.gap_R * coll.sigma[0][1]) + coll.co[0][1], atol=1e-6)
            assert np.isclose(coll.jaw_L, (coll.gap_L * coll.sigma[0][0]) + coll.co[0][0], atol=1e-6)
        ...
        assert np.isclose(coll.jaw_R, (coll.gap_R * coll.sigma[0][1]) + coll.co[0][1], atol=1e-6)
    elif succession[0].endswith('L') and succession[1].endswith('L'):
        only_L = True
        assert coll.gap_R == None
        assert coll.jaw_R == None
        assert np.isclose(coll.jaw_L, (coll.gap_L * coll.sigma[0][0]) + coll.co[0][0], atol=1e-6)
    elif (succession[0]!='angle' and succession[1]!='angle' 
          and succession[0]!='tilt' and succession[1]!='tilt'):
        assert coll.jaw_L > coll.jaw_R
        assert coll.gap_L > coll.gap_R
        assert np.isclose(coll.jaw_L, (coll.gap_L * coll.sigma[0][0]) + coll.co[0][0], atol=1e-6)
        assert np.isclose(coll.gap_R,(coll.jaw_R - coll.co[0][1]) / coll.sigma[0][1], atol=1e-6)
        assert np.isclose(coll.jaw_R, (coll.gap_R * coll.sigma[0][1]) + coll.co[0][1], atol=1e-6)
        assert np.isclose(coll.gap_L,(coll.jaw_L - coll.co[0][0]) / coll.sigma[0][0], atol=1e-6)
    if succession[0] == 'angle' or succession[1] == 'angle' or succession[0] == 'tilt' or succession[1] == 'tilt':
    ...
    # 2nd level
    print(f" {coll.gap_L} {coll.gap_R} {coll.jaw_L} {coll.jaw_R}")
    ...
    #     assert np.isclose(coll.gap_L, (coll.jaw_L - coll.co[0][0]) / coll.sigma[0][0], atol=1e-6)
    #     assert np.isclose(coll.gap_R,(coll.jaw_R - coll.co[0][1]) / coll.sigma[0][1], atol=1e-6)
    #     assert ~np.any(gaps_and_jaws_check > 100)
    #     assert coll.jaw_L > coll.jaw_R and coll.gap_L > coll.gap_R
    # elif check_L == True:
    #     assert np.isclose(coll.gap_L, (coll.jaw_L - coll.co[0][0]) / coll.sigma[0][0], atol=1e-6)
    # elif check_R == True:
    #     assert np.isclose(coll.gap_R,(coll.jaw_R - coll.co[0][1]) / coll.sigma[0][1], atol=1e-6)
