...
def test_fluka():
    length = 1.2
    num_slices = 2000
    coll = xc.FlukaCollimator(length=length/num_slices, jaw=0.001, material='Ti')
    xc.fluka.engine.particle_ref = particle_ref

    with flaky_assertions():
        xc.fluka.engine.start(elements=coll)
