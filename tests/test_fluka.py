...
def test_fluka_deep_check(log_impacts, mark_scattered_particles):
    ...
    # Get all children linked to their parents
    mask_child  = part_mid.particle_id > part_init.particle_id.max()
    child_id    = part_mid.particle_id[mask_child]
    tree        = xc.ParticlesTree(part_mid)
    child_idx   = tree.indices_of(child_id)  # vectorised lookup
    primary_idx = tree.root_idx[child_idx]   # roots per child, fast
    primary_id  = tree.root_ids[child_idx]

    # And check that the sum of the children energy and leftover energy of the parent is close to the initial energy
    ...
    if log_impacts:
        ...
        assert np.all([pid in df_mid.id_before.values for pid in part_mid.particle_id[mask_hitbox_but_miss]])
        assert np.all([pid in df_mid.id_before.values for pid in part_mid.particle_id[mask_hit]])
        df_mid = df_mid.sort_values("id_before")
        assert np.all(part_black_mid.particle_id[mask_hit | mask_hitbox_but_miss] == df_mid.id_before.values)
    ...
    # Get all children linked to their parents
    mask_child  = part.particle_id > part_init.particle_id.max()
    ...
    for i, (pid, e_parent) in enumerate(zip(part.particle_id[mask_hit], part.energy[mask_hit])):
        des = tree.descendants_ids(pid)
        if len(des) > 0:
