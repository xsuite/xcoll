...
int64_t ParticleStatsMonitor_get_slot(ParticleStatsMonitorData el, LocalParticle* part){
    ...
    if (part_id_end < 0 || (part_id_start <= particle_id && particle_id < part_id_end)){
        ...
        int64_t const max_slot = ParticleStatsMonitorRecord_len_count(record);

        // Compute sample index
        slot = round(sampling_frequency * ( (at_turn-start_at_turn)/frev - zeta/beta0/C_LIGHT ));
        if (slot >= max_slot){
            slot = 0;
        }
    }
...
void ParticleStatsMonitor_track_local_particle(ParticleStatsMonitorData el, LocalParticle* part0){
    ...
    START_PER_PARTICLE_BLOCK(part0, part);
        ...
        if (slot >= 0){
            ...
            ParticleStatsMonitorData_set_cached(el, slot, 0);
            ...
            if (monitor_zeta){ zeta = LocalParticle_get_zeta(part); }
            if (monitor_pzeta){
