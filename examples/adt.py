...
_, ax = plt.subplots(figsize=(6,4))
ax.fill_between(mon.turns, 1e3*mon.x_mean + 1e3*mon.x_std, 1e3*mon.x_mean - 1e3*mon.x_std, alpha=0.4)
ax.plot(mon.turns, 1e3*mon.x_mean, label=r'$<x>$')
ax.axvline(adt_turns, c='r', ls='--', label='stop blow-up')
...
_, ax = plt.subplots(figsize=(6,4))
ax.fill_between(mon.turns, 1e3*mon.y_mean + 1e3*mon.y_std, 1e3*mon.y_mean - 1e3*mon.y_std, alpha=0.4)
ax.plot(mon.turns, 100*mon.y_mean, label=r'$<y>$')
ax.axvline(adt_turns, c='r', ls='--', label='stop blow-up')
ax.set_ylabel(r"normalised amplitude $[mm]$")
ax.set_xlabel("Turn number")
ax.legend()
