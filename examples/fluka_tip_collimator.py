...
xc.fluka.engine.start(elements=[coll1, coll2], clean=False, verbose=False, fortran_debug_level=1)
...
# Do the tracking in FLUKA with tip
print(f"Tracking {num_part} particles (FLUKA with tip)...     ", end='', flush=True)
start = time.time()
coll1.track(part1)
print(f"Done in {round(time.time()-start, 3)}s.")

# Do the tracking in FLUKA without tip
print(f"Tracking {num_part} particles (FLUKA without tip)...   ", end='', flush=True)
start = time.time()
coll2.track(part2)
print(f"Done in {round(1000*(time.time()-start), 3)}ms")

print(f"Survived in FLUKA with tip: {len(part1.state[part1.state>0])}/{num_part}")
...
ax[0, 0].set_title('FLUKA WITH TIP')
